...
#### 接口设计
```
GET    /api/dataflows              # 获取所有数据流
POST   /api/dataflows              # 创建数据流
PUT    /api/dataflows/{id}         # 更新数据流
DELETE /api/dataflows/{id}         # 删除数据流
...
#### 接口设计补充
```
POST   /api/data/import/local          # 导入本地CSV/Excel文件
```
...
#### 接口设计
```
POST   /api/analysis/aggregate    # 多表聚合
...
#### 4.11.1 3D形貌图（Surface3DChart）
- 功能：展示3D表面数据
- 技术：echarts-gl的surface类型
- 数据格式：二维数组 [[z1, z2, ...], ...]
- 特点：支持视角旋转、缩放

#### 4.11.2 联动图表（LinkedChart）
- 功能：折线图和饼图联动，鼠标悬停时同步高亮
- 技术：ECharts的updateAxisPointer事件
- 特点：共享数据集，视觉联动效果

#### 4.11.3 LED晶圆分析图（LEDWaferChart）
